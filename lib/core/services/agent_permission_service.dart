...
///
/// Note: This is distinct from [PermissionService] (in permission_service.dart),
/// which handles OS-level permissions (camera, storage, etc.) via the
/// `permission_handler` package. This class manages the Agent's tool
/// authorization policy.
class AgentPermissionService extends ChangeNotifier {
  ...
  /// Creates an [AgentPermissionService] with the given initial mode.
  AgentPermissionService({PermissionMode initialMode = PermissionMode.normal})
      : _mode = initialMode;
  ...
  void setMode(PermissionMode mode) {
    ...
    _mode = mode;
    debugPrint('[AgentPermissionService] mode changed to: ${mode.name}');
    notifyListeners();
  }
